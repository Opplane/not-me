...
> extends BaseSchema<BaseType, {[key: string]: InferType<ValuesSchemas[number]>}, NT> {
  constructor(valuesSchema: ValuesSchemas, message?: string, fieldDoesNotMatch?: string) {
    ...
    this.addShapeFilter((input, options) => {
      ...
      for (const fieldKey in input) {
        ...
        let validFieldResult: { invalid: false, value: unknown } | undefined = undefined
        ...
        if (!validFieldResult) {
          const messages = [fieldDoesNotMatch || DefaultInvalidationMessagesManager.getDefaultMessages()?.objectOf?.fieldDoesNotMatch || 'Did not match any of the provided schemas']
...
export function object<ValuesSchemas extends ValuesSchemasBase>(
  valuesSchema: ValuesSchemas,
  message?: string,
  fieldDoesNotMatch?: string
): ObjectOfSchema<ValuesSchemas> {
  return new ObjectOfSchema<ValuesSchemas>(valuesSchema, message, fieldDoesNotMatch);
}